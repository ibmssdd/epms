...
class CmtTaskGenerator {
  CmtTaskGenerator({required Database db, DateTime Function()? now})
    : _db = db,
  ...
  static const String _pcbSubjectTaskId = 'PCB_CMT_TEST';
  ...
  ///   - Uses PCB_CMT_TEST.
  Future<Map<String, int>> generateCmtTasks({DateTime? milestoneDate}) async {
    ...
    if (milestone == null) {
      debugPrint('CMT milestone NOT FOUND.');

      return const {'PHY': 0, 'CHEM': 0, 'BIO': 0, 'PCB': 0};
    }

    debugPrint('CMT milestone found.');
  ...
  Future<int> _processSubject({
    ...
    debugPrint('');
    debugPrint('------------------------------------------------');
    debugPrint('PROCESSING SUBJECT: $subjectCode');
    debugPrint('------------------------------------------------');
    ...
    debugPrint('Selected chapters: $chapters');

    if (chapters.isEmpty) {
      debugPrint('$subjectCode: no chapter scope. Result = 0');
    ...
    final rules = await _getCmtRulesForSubject(subjectCode);

    debugPrint('Active rules found: ${rules.length}');

    if (rules.isEmpty) {
      debugPrint('$subjectCode: no active task creation rules. Result = 0');
    ...
    for (final rule in rules) {
      ...
      debugPrint('');
      debugPrint('==============================================');
      debugPrint('$subjectCode CMT RULE');
      ...
      debugPrint('SubjectTask partial key: $partialKey');
  ...
  Future<int> _processCommonTasks({
    ...
    debugPrint('PCB is milestone-level and does not require chapter scope.');

    final subjectTask = await _getSubjectTaskById(_pcbSubjectTaskId);
    ...
    final activities = await _getActivitiesForSubjectTask(_pcbSubjectTaskId);
    ...
    if (activities.isEmpty) {
      debugPrint('PCB has no active activities.');

      return 0;
    }

    final taskDescription =
        _string(subjectTask[_subjectTaskNameColumn]) ?? _pcbSubjectTaskId;
    ...
    for (var i = 0; i < activities.length; i++) {
      final activityId = _string(activities[i]['ActivityID']);

      final activityDescription = _activityDescription(activities[i]);
    ...
    debugPrint('');
    debugPrint('PCB TaskID: $taskId');
    debugPrint('PCB Duration: $duration minutes');
    ...
    if (result) {
      ...
      debugPrint('PCB/common milestone task-created flag = 1');

      debugPrint('PCB RESULT = 1 (CREATED/PRESENT)');
  ...
  Future<bool> _createTaskFromSubjectTask({
    ...
    if (activities.isEmpty) {
      debugPrint('No active activities for $subjectTaskId');
    ...
    debugPrint('');
    debugPrint('Scope: ${chapterCodes.join(', ')}');
  ...
  Future<bool> _createTaskAndActivityStatus({
    ...
    if (!alreadyExists) {
      ...
      debugPrint('TaskLog inserted: $taskId');
    } else {
      debugPrint('Task already exists: $taskId');
    }
    ...
    if (existingStatus.isEmpty) {
      ...
      debugPrint('Activity status inserted: $taskId');
    } else {
      debugPrint('Activity status already exists: $taskId');
    }
  ...
  Future<List<Map<String, Object?>>> _getCmtRulesForSubject(
    ...
    debugPrint('');
    debugPrint('RULE LOOKUP: $normalizedSubject');
    ...
    for (final row in rows) {
      ...
      if (normalizedRuleSubject == normalizedSubject) {
        debugPrint('  -> MATCHED $normalizedSubject');
  ...
  Future<List<Map<String, Object?>>> _getSubjectTasksByPartialKey(
    ...
    debugPrint('SubjectTasks found: ${rows.length}');
