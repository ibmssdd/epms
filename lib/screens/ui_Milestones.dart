...
MilestoneCalendarSvc.nextSunday(DateTime.now());
...
Future<DateTime> _getNextAvailableSundayForType(
...
return _getNextAvailableSunday(
from: today,
occupiedDates: occupiedDates,
);
}
...
Future<List<_ChapterOption>> loadSubject(
String subjectCode,
) async {
...
void _loadSelectionsFromMilestone(
...
taskResults =
await _generateTasksForSavedMilestone();
}
...
final commonTaskStatus =
results['PCB'] ?? 0;

final commonTaskText =
commonTaskStatus == 1
...
Set<String> _lockedSelectionFor(
String subjectCode,
) {
switch (subjectCode) {
case 'PHY':
...
List<_ChapterOption> _chaptersFor(
String subjectCode,
) {
switch (subjectCode) {
case 'PHY':
...
return _chaptersFor(subjectCode)
    .where((chapter) {
final number =
_chapterNumber(chapter.code);
...
CrossAxisAlignment.stretch,
children: [
_buildMilestoneEntryHeader(
context,
),
const SizedBox(height: 8),
_buildSubjectTabs(context),
const SizedBox(height: 6),
_buildSelectedSubjectContent(
context,
),
...
Widget _buildMilestoneEntryHeader(
BuildContext context,
) {
final colors =
Theme.of(context).colorScheme;
...
Widget _buildSelectedSubjectContent(
BuildContext context,
) {
final subject =
